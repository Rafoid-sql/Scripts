...
FROM
		...
		FROM
				...
				FROM
						( SELECT B.NAME GNAME, A.PARENT_INDEX  PINDEX, A.NAME ANAME, A.REFERENCE_INDEX RINDEX , A.SYSTEM_CREATED, C.TYPE FILE_TYPE, C.SPACE
						FROM V$ASM_ALIAS A, V$ASM_DISKGROUP B, V$ASM_FILE C
						WHERE A.GROUP_NUMBER = B.GROUP_NUMBER AND A.GROUP_NUMBER = C.GROUP_NUMBER(+) AND A.FILE_NUMBER = C.FILE_NUMBER(+) AND A.FILE_INCARNATION = C.INCARNATION(+) ) 
				START WITH 
				(MOD(PINDEX, POWER(2, 24))) = 0 AND RINDEX IN
				( SELECT A.REFERENCE_INDEX
				FROM V$ASM_ALIAS A, V$ASM_DISKGROUP B
				WHERE   A.GROUP_NUMBER = B.GROUP_NUMBER AND    MOD(A.PARENT_INDEX, POWER(2, 24)) = 0) 
				CONNECT BY 
				PRIOR RINDEX = PINDEX )
