...
EXEC_ATUAL=TkAtualizaStandby.sh
ORACLE_SID_ORG=tasy
...
echo [`date '+%X %x'`] " -> GERANDO CRONTAB TEMPORARIO" >> $ARQ_LOG
  ...
  crontab -l > $BASE_DIR/crontab.org
echo [`date '+%X %x'`] " -> DESATIVANDO BACKUP DE ARCHIVES NA CRON" >> $ARQ_LOG
  crontab -l > $BASE_DIR/crontab2.org
echo "sed '/$EXEC_ATUAL/s/^/#CLONEDB#/' $BASE_DIR/crontab2.org > $BASE_DIR/crontab2.tmp" > $BASE_DIR/crontab2.sh
  sh $BASE_DIR/crontab2.sh
  crontab $BASE_DIR/crontab2.tmp
# Inicia Monitoramento do Target e Auxiliary via icmp
#nohup sh $CLONE_HOME/script/TK_Mon_Server.sh &
