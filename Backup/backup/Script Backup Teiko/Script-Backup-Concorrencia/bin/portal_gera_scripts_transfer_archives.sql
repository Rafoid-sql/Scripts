...
SPOOL '/home/oracle/scripts/bin/PORTAL_TRANSFERE_ARCHIVE_TO_STANDBY.sh';
select distinct  'asmcmd cp ' ||name || ' /u02/portal/archives'||substr(name,24,50)
from gv$archived_log
where
DELETED = 'NO' and resetlogs_change# = '20536176394' and
not exists (select sequence from transfer_arc.transfered_files
           where sequence# = sequence and thread# = thread)
order by 1;

SPOOL off;
SPOOL '/home/oracle/scripts/bin/PORTAL_GRAVA_LOG_ARCHIVE_TRANSFERIDO.sql';
SELECT * FROM (
