...
SPOOL '/home/oracle/scripts/bin/PRODUCAO_TRANSFERE_ARCHIVE_TO_STANDBY.sh';
select distinct  'asmcmd cp ' ||name || ' /u02/producao/archives'||substr(name,26,50)
from gv$archived_log
where
DELETED = 'NO' and resetlogs_change# = '20536177346' and
not exists (select sequence from transfer_arc.transfered_files
           where sequence# = sequence and thread# = thread)
order by 1;

SPOOL off;
SPOOL '/home/oracle/scripts/bin/PRODUCAO_GRAVA_LOG_ARCHIVE_TRANSFERIDO.sql';
SELECT * FROM (
