...
do
  ...
  DISPLAY "Expdp do Usuáo..............: ${USUARIO}"
  DISPLAY "Inicio........................: `date '+%d/%m/%Y %T'`"

  expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=DATA_PUMP FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} SCHEMAS='\"'${USUARIO}'\"' DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} filesize=10000M CLUSTER=N  < /dev/null 2>${TMP_FILE2}
...
EXPORT_FULL(){
  ...
  expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=DATA_PUMP FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} FULL=Y DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} filesize=10000M < /dev/null 2>${TMP_FILE2}
...
if  [ $servername = "PRD2132" ]
then
        export DIR_BASE=/u03/backup/${INSTANCE}/logico/
