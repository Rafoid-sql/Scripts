...
${ORACLE_HOME}/bin/sqlplus -s ${USER_EXPORT}/$USER_EXPORT_PASSWORD@${INSTANCE} 2>&1 <<EOF
whenever sqlerror exit sql.sqlcode ;
insert into tk_bkp_exclui_tabela (owner,tabela)
select owner, table_name as tabela from (
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'ARQ%' and table_name not in ('ARQTEMPPRD','ARQTEMP','ARQREQDEV','ARQITEM')
union
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'TEMP%' and table_name not in ('TEMPPROC','TEMPIMP')
union
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'AGEEXATMP%' and table_name not in ('AGEEXATMP')
union
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'TMP%' and table_name not in ('TMP')
union
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'TT%' and table_name not in ('TT')
union
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'WK%' and table_name not in ('WK')
union
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'ENTSAI%' and table_name not in ('ENTSAI')
union
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'LEITOS%' and table_name not in ('LEITOS')
union
select owner,table_name from dba_tables where owner='WPD' and  table_name like 'CRITICA%' and table_name not in ('CRITICA')
)
where table_name not in (select tabela from tk_bkp_exclui_tabela);
commit;
exit
EOF
}
...
REMOTO (){
       ...
       then
          ...
          find /orabackup_rmt/backup/hbprd1/datapump/files*.dmp.gz -mtime +5 -print -exec rm -f {} \; > /dev/null 2>&1
          find ${DIR_DMP} -maxdepth 1 -name '*.dmp.gz*' -exec cp -uv {} /orabackup_rmt/backup/hbprd1/datapump/files/ \;
          find ${DIR_DMP}/*.gz -exec rm -f {} \; > /dev/null 2>&1
          ret=$?
          if [ "${ret}" != "0" ]
          then
...
#Faz a inclusão de tabelas a serem excluídas na tabela TK_BKP_EXCLUI_TABELA
  INSERIR_TAB_EXC

  expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=DATA_PUMP FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} SCHEMAS='\"'${USUARIO}'\"' DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} filesize=8000M  exclude=statistics EXCLUDE=TABLE:\"IN \(SELECT TABELA FROM TK_BKP_EXCLUI_TABELA WHERE OWNER IN \(SELECT OWNER FROM TK_BKP_EXCLUI_TABELA\)\)\" < /dev/null 2>${TMP_FILE2}

else

  expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=DATA_PUMP FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} SCHEMAS='\"'${USUARIO}'\"' DUMPFILE=${ARQ_D    MP} logfile=${ARQ_LOG} filesize=8000M exclude=statistics < /dev/null 2>${TMP_FILE2}
...
EXPORT_FULL(){
  ...
  find ${DIR_DMP}/*.gz -print -exec rm -f {} \; > /dev/null 2>&1
...
then
  ...
  INSERIR_TAB_EXC
        ...
        expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=${DIR_DP} FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} FULL=Y DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} filesize=8000M exclude=statistics EXCLUDE=TABLE:\"IN \(SELECT TABELA FROM TK_BKP_EXCLUI_TABELA WHERE OWNER IN \(SELECT OWNER FROM TK_BKP_EXCLUI_TABELA\)\)\" < /dev/null 2>${TMP_FILE2}
else

        expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=${DIR_DP} FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} FULL=Y DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} filesize=8000M exclude=statistics < /dev/null 2>${TMP_FILE2}
...
if  [ $servername = "asklepio1.hospitalbalbino.com.br" ]
then
        #export DIR_BASE=/backup02/hbprd1/datapump
        #export DIR_DP=DATA_PUMP_2
        export DIR_DP=DATA_PUMP
        export DIR_BASE=/backup/hbprd1/datapump
...
find ${DIR_LOG}/* -mtime +10 -print -exec rm -f {} \; > /dev/null 2>&1
#find ${DIR_DMP}/*.dmp -mtime +${RETENCAO_DMP} -print -exec rm -f {} \; > /dev/null 2>&1
#find ${DIR_DMP}/*.dmp.gz -mtime +${RETENCAO_DMP} -print -exec rm -f {} \; > /dev/null 1>&1
