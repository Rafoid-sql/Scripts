#[oracle@dbserver4:dbprod2 log]$ cat /orabackup/datapump/script/BackupExpDpNode4MVPROD.sh
#set -x
#!/bin/ksh
#######################################################
...
achou=0
if [ "$(cat ${DIR_DMP}/${ARQ_LOG} | grep "ORA-31693")" != "" ]; then
cat ${DIR_DMP}/${ARQ_LOG} | grep "ORA-31693" > ${ERRO}
        if [ "${TIPO}" = "OWNER" ]; then
...
do
  ...
  #expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=DATA_PUMP FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} SCHEMAS='\"'${USUARIO}'\"' DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} filesize=5000M < /dev/null 2>${TMP_FILE2}
  if [ "${USUARIO}" = "DBAMV" ];
    then
       expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=DATA_PUMP FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} SCHEMAS='\"'${USUARIO}'\"' DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} EXCLUDE=TABLE:\"IN \(\'ACESSO_PAGINA\', \'ARQUIVO_DOCUMENTO\', \'ACESSO_DOCUMENTO\'\)\" filesize=5000M < /dev/null 2>${TMP_FILE2}
  else
       expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=DATA_PUMP FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} SCHEMAS='\"'${USUARIO}'\"' DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} filesize=5000M < /dev/null 2>${TMP_FILE2}
  fi
...
EXPORT_FULL(){
  ...
  expdp userid=${USER_EXPORT}/${USER_EXPORT_PASSWORD}@$INSTANCE DIRECTORY=DATA_PUMP FLASHBACK_SCN=$SCN VERSION=${ORACLE_VERSION} FULL=Y DUMPFILE=${ARQ_DMP} logfile=${ARQ_LOG} filesize=5000M < /dev/null 2>${TMP_FILE2}
...
if  [ $servername = "dbserver4.unimedcco.unimedchapeco.com.br" ]
then
        export DIR_BASE=/orabackup/datapump/${ORACLE_SID}
