...
if [ $INS_COUNT -eq 1 ]
 then
   export ORACLE_SID=$( ps -ef|grep pmon|grep -v grep|egrep -v ${SKIPDBS}|awk '{print $NF}'|sed -e 's/ora_pmon_//g'|grep -v sed|grep -v "s///g" )
...
# If there is more than one DB ASK the user to select:
 ...
 then
    ...
    echo "-----------------------------------------"
    select DB_ID in $( ps -ef|grep pmon|grep -v grep|egrep -v ${SKIPDBS}|awk '{print $NF}'|sed -e 's/ora_pmon_//g'|grep -v sed|grep -v "s///g" )
     do
        if [ -z "${REPLY##[0-9]*}" ]
         then
