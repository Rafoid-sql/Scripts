...
--WHERE TBM.TABLESPACE_NAME in ('RECON_DATA','GROUP_BUSINESS_SOLUTIONS_DATA')
ORDER BY "%FULL" ASC;
=========================================================================================================================================
--UNDO SPACE USAGE
...
ON TOTAL.TS = DBAT.TABLESPACE_NAME
LEFT JOIN 
    (SELECT TABLESPACE_NAME, MAX(DECODE(AUTOEXTENSIBLE, 'YES', 'YES', 'NO')) AUTOEXTEND, MAX(DECODE(MAXBYTES, 0, 0, MAXBYTES) / 1024 / 1024) MAXSIZE, MAX(DECODE(AUTOEXTENSIBLE, 'YES', INCREMENT_BY * (SELECT VALUE FROM V$PARAMETER WHERE NAME = 'db_block_size') / 1024 / 1024, 0)) ON_NEXT
     FROM DBA_DATA_FILES 
     GROUP BY TABLESPACE_NAME) FILEINFO
