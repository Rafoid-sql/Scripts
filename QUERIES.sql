...
SET LINES 300
SET PAGESIZE 2000
SELECT SUBSTR(A.SPID,1,9) PID, SUBSTR(B.SID,1,5) SID, SUBSTR(B.SERIAL#,1,5) SER#,SUBSTR(B.PROCESS,1,25) PROC , SUBSTR(B.MACHINE,1,25) BOX, SUBSTR(B.USERNAME,1,12) USERNAME, SUBSTR(B.CLIENT_INFO,1,40) CLIENT, SUBSTR(B.OSUSER,1,20) OS_USER, SUBSTR(B.PROGRAM,1,40) PROGRAM, SUBSTR(B.STATUS,1,10) STATUS 
FROM V$SESSION B, V$PROCESS A 
WHERE B.PADDR = A.ADDR 
AND TYPE='USER'
--AND B.USERNAME IN ('LB2_BKP')
...
SELECT S.USERNAME USERNAME,S.SID SID,S.SERIAL# SERIAL$,S.OSUSER OSUSER,S.LAST_CALL_ET/60 MINS_RUNNING,Q.SQL_TEXT SQL_TEXT
...
SELECT NVL(S.USERNAME, '(ORACLE)') AS USERNAME, S.SID, S.SERIAL#, SW.EVENT, SW.WAIT_CLASS, SW.WAIT_TIME, SW.SECONDS_IN_WAIT, SW.STATE
FROM V$SESSION_WAIT SW, V$SESSION S
WHERE  S.SID = SW.SID 
ORDER BY SW.SID,SW.SECONDS_IN_WAIT DESC;
