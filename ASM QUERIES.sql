--VERIFICAR TAMANHO DO DISKGROUP

--ASM DISK size query checking
--1)  Lun size check for ASM Disk size.. 
SELECT D.GROUP_NUMBER, G.NAME, D.OS_MB/1024 GB FROM V$ASM_DISK D LEFT OUTER JOIN V$ASM_DISKGROUP G ON (D.GROUP_NUMBER = G.GROUP_NUMBER) ORDER BY G.GROUP_NUMBER, D.OS_MB;

--2) Asm Disk Group Size..
SET LINESIZE 1000
BREAK ON REPORT ON DISK_GROUP_NAME SKIP 1
COMPUTE SUM LABEL "GRAND TOTAL: " OF TOTAL_MB USED_MB ON REPORT
SELECT NAME GROUP_NAME, ROUND(TOTAL_MB) TOTAL_MB, ROUND((TOTAL_MB - FREE_MB)) USED_MB, ROUND(FREE_MB) FREE_MB, ROUND((1-(FREE_MB/TOTAL_MB))*100,2) USED, (100-ROUND((1-(FREE_MB/TOTAL_MB))*100,2)) FREE FROM V$ASM_DISKGROUP ORDER BY NAME;

-- ASM SPECIFIG DISKGROUP:
SET LINESIZE 1000
SELECT NAME GROUP_NAME, ROUND(TOTAL_MB) TOTAL_MB, ROUND(FREE_MB) FREE_MB, ROUND((1-(FREE_MB/TOTAL_MB))*100,2) USED, (100-ROUND((1-(FREE_MB/TOTAL_MB))*100,2)) FREE FROM V$ASM_DISKGROUP WHERE NAME ='DATAC1';
