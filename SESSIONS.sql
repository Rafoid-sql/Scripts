...
COL TODAY_STATUS FOR A15
COL TODAY_START_TIME FOR A40
COL RUN_DURATION FOR A15
SELECT J.JOB_NAME,DECODE(J.ENABLED, 'TRUE', 'T', 'FALSE', 'F', J.ENABLED) AS "E",J.STATE,J.NEXT_RUN_DATE,R.STATUS AS TODAY_STATUS,R.ACTUAL_START_DATE AS TODAY_START_TIME,R.RUN_DURATION,R.ERRORS
FROM DBA_SCHEDULER_JOBS J
LEFT JOIN DBA_SCHEDULER_JOB_RUN_DETAILS R ON J.JOB_NAME = R.JOB_NAME AND R.ACTUAL_START_DATE >= TRUNC(SYSDATE)
WHERE J.JOB_NAME IN ('SKU_LTC_CONFIG_CLEANUP_JOB', 'SKU_DOWNPAYMENT_CLEANUP_JOB')
ORDER BY J.JOB_NAME, R.ACTUAL_START_DATE DESC;
=========================================================================================================================================
...
COL NODE FOR 99
COL FROM_WHERE FOR A30
COL SID_SER FOR A12
COL PROGRAM FOR A35
COL OSUSER FOR A12
COL SQL_TEXT FOR A65
COL BLK_SESS FOR 99999
COL FROM_WHEN FOR A25
...
BLOCKING_SESSION BLK_SESS,TO_CHAR(LOGON_TIME) FROM_WHEN
FROM GV$SESSION SES
WHERE TYPE IN ('USER')
--AND OSUSER NOT IN ('weblogiccdes')
--AND PROGRAM NOT LIKE ('JDBC Thin Client')
--AND SCHEMANAME = ('SVC_QAT_EMMT_FAL_MS_06')
--AND SQL_ID IN ('frccccnh76gtx')
--AND SID IN (110)
AND NVL((SELECT DISTINCT SQL_TEXT FROM GV$SQL SQL WHERE SQL.SQL_ID = SES.SQL_ID),'NOTHING GOING ON') NOT LIKE '%NOTHING GOING ON%'
--AND STATUS = 'INACTIVE'
--AND NVL((SELECT DISTINCT SQL_TEXT FROM GV$SQL SQL WHERE SQL.SQL_ID = SES.SQL_ID),'NOTHING GOING ON') LIKE '%VW_NE_ONYX_COMBINED%'
...
COL BLK_SESS FOR 99999
COL FROM_WHEN FOR A25
SELECT SID||','||SERIAL#||'@'||INST_ID SID_SER, SQL_ID, STATUS, OSUSER||':'||SCHEMANAME||'@'||SERVICE_NAME FROM_WHERE, TO_CHAR(LOGON_TIME) FROM_WHEN, BLOCKING_SESSION BLK_SESS, NVL((SELECT DISTINCT SQL_TEXT FROM GV$SQL SQL WHERE SQL.SQL_ID = SES.SQL_ID),'NOTHING GOING ON') "SQL_TEXT"
FROM GV$SESSION SES
WHERE TYPE IN ('USER')
AND SID IN (1145,1362,1108,717,1488,1457,1487,1450,1194,7)
AND NVL((SELECT DISTINCT SQL_TEXT FROM GV$SQL SQL WHERE SQL.SQL_ID = SES.SQL_ID),'NOTHING GOING ON') NOT LIKE '%NOTHING GOING ON%'
--AND STATUS = 'INACTIVE'
--AND NVL((SELECT DISTINCT SQL_TEXT FROM GV$SQL SQL WHERE SQL.SQL_ID = SES.SQL_ID),'NOTHING GOING ON') LIKE '%VW_NE_ONYX_COMBINED%'
ORDER BY STATUS,OSUSER,PROGRAM;
=========================================================================================================================================
-- Check Session Details III
COL NODE FOR 99
...
WHERE TYPE = 'USER'
--AND SID IN (1442)
--AND SCHEMANAME = 'ANONYMOUS'
--AND STATUS = 'INACTIVE'
--AND SQL_ID='44g45pvrqwbb8'
AND BLOCKING_SESSION IS NOT NULL
ORDER BY BLOCKING_SESSION,FROM_WHEN,STATUS,OSUSER,PROGRAM;
=========================================================================================================================================
...
AND S.SQL_ID = '44g45pvrqwbb8'
--AND S.SID IN (1286,645)
--AND L.SID IN (1286,645)
ORDER BY STATUS, OSUSER, PROGRAM;
=========================================================================================================================================
...
WHERE "%_COMP" <> 100
--WHERE L.SQL_ID = A.SQL_ID AND L.SQL_ID = S.SQL_ID AND L.SID = S.SID AND TYPE = 'USER' AND TOTALWORK > 0 AND SOFAR != TOTALWORK
--AND SQL_ID = '572ky2y7f1y6a'
AND SID IN (1286,645)
ORDER BY STATUS, OSUSER, PROGRAM;
=========================================================================================================================================
--Check time to finish V
COL NODE FOR 99
COL FROM_WHERE FOR A45
COL "SESSION" FOR A13
COL PROGRAM FOR A40
COL OPNAME FOR A15
COL "BLK_SESS" FOR 99999999
COL "%_COMPLETE" FOR 999.99
SELECT SES.SID||','||SES.SERIAL#||'@'||SES.INST_ID "SESSION", SES.SQL_ID, SES.STATUS, TO_CHAR(SES.LOGON_TIME, 'YYYY-MM-DD HH24:MI:SS') AS LOGON_TIME, SES.OSUSER||':'||SES.SCHEMANAME||'@'||SES.SERVICE_NAME FROM_WHERE, SES.PROGRAM, SES.BLOCKING_SESSION "BLK_SESS", LOP.OPNAME,
CASE
	WHEN LOP.TOTALWORK = 0 THEN NULL
	ELSE ROUND(LOP.SOFAR / LOP.TOTALWORK * 100, 2)
END AS "%_COMPLETE"
FROM GV$SESSION SES
LEFT JOIN GV$SESSION_LONGOPS LOP ON SES.SID = LOP.SID AND SES.SERIAL# = LOP.SERIAL# AND SES.INST_ID = LOP.INST_ID AND LOP.SOFAR <> LOP.TOTALWORK
WHERE SES.TYPE = 'USER' AND SES.SCHEMANAME NOT IN (SELECT USERNAME FROM DBA_USERS WHERE ORACLE_MAINTAINED='Y')
AND SES.SID IN (1529,622)
--AND SES.SCHEMANAME = 'RECON'
--AND SES.STATUS = 'ACTIVE'
ORDER BY LOGON_TIME, SES.STATUS, SES.OSUSER, SES.PROGRAM;
=========================================================================================================================================
--CHECK TIME TO FINISH VI
COL NODE FOR 99
COL FROM_WHERE FOR A45
COL "SESSION" FOR A14
COL ST FOR A2
...
COL OPNAME FOR A15
COL "BLK_SESS" FOR 99999999
COL "%_COMP" FOR 999.99
SELECT
CASE
    WHEN PX.QCSID IS NOT NULL THEN LPAD(' ', 2) || SES.SID||','||SES.SERIAL#||'@'||SES.INST_ID
    ELSE SES.SID||','||SES.SERIAL#||'@'||SES.INST_ID
END AS "SESSION",
CASE SES.STATUS
    WHEN 'ACTIVE'   THEN 'A'
    WHEN 'INACTIVE' THEN 'I'
    ELSE SUBSTR(SES.STATUS, 1, 1)
END AS ST, SES.SQL_ID, TO_CHAR(SES.LOGON_TIME, 'YYYY-MM-DD HH24:MI:SS') AS LOGON_TIME, UPPER(SES.OSUSER)||':'||SES.SCHEMANAME||'@'||SES.SERVICE_NAME FROM_WHERE, SES.PROGRAM, SES.BLOCKING_SESSION "BLK_SESS", LOP.OPNAME,
CASE
    WHEN LOP.TOTALWORK = 0 THEN NULL
    ELSE LEAST(ROUND(LOP.SOFAR / LOP.TOTALWORK * 100, 2), 100)
END AS "%_COMP"
FROM GV$SESSION SES
LEFT JOIN GV$SESSION_LONGOPS LOP ON SES.SID = LOP.SID AND SES.SERIAL# = LOP.SERIAL# AND SES.INST_ID = LOP.INST_ID AND LOP.SOFAR <> LOP.TOTALWORK
LEFT JOIN GV$PX_SESSION PX ON SES.SID = PX.SID AND SES.SERIAL# = PX.SERIAL# AND SES.INST_ID = PX.INST_ID
WHERE SES.TYPE = 'USER' AND SES.SCHEMANAME NOT IN (SELECT USERNAME FROM DBA_USERS WHERE ORACLE_MAINTAINED = 'Y' AND USERNAME <> 'SYS')
    --AND SES.SID IN (362)
    --AND SES.SCHEMANAME = 'RECON'
    --AND SES.STATUS = 'ACTIVE'
ORDER BY NVL(PX.QCSID, SES.SID), NVL(PX.QCINST_ID, SES.INST_ID), CASE WHEN PX.QCSID IS NULL THEN 0 ELSE 1 END, SES.SID;
=========================================================================================================================================
--CHECK TIME TO FINISH VII
COL NODE FOR 99
COL FROM_WHERE FOR A45
COL "SESSION" FOR A14
COL ST FOR A2
...
DECLARE
    ...
    V_UBLK_END     NUMBER;
    V_RATE         NUMBER;
    V_SECONDS_LEFT NUMBER;
    V_SID          NUMBER := 110;   -- Update with your SID
    V_SERIAL       NUMBER := 28566; -- Update with your SERIAL#
BEGIN
    SELECT USED_UBLK INTO V_UBLK_START FROM GV$TRANSACTION T, GV$SESSION S WHERE T.SES_ADDR = S.SADDR AND S.SID = V_SID AND S.SERIAL# = V_SERIAL; --1st snapshot
    DBMS_OUTPUT.PUT_LINE('Measuring rollback rate... (waiting 10 seconds)');
	--DBMS_LOCK.SLEEP(10) -- 10 second sleep (18c or older)
    DBMS_SESSION.SLEEP(10); -- 10 second sleep (19c or newer)
    SELECT USED_UBLK INTO V_UBLK_END FROM GV$TRANSACTION T, GV$SESSION S WHERE T.SES_ADDR = S.SADDR AND S.SID = V_SID AND S.SERIAL# = V_SERIAL; --2nd snapshot
    V_RATE := (V_UBLK_START - V_UBLK_END) / 10; --Calculate Rate and ETA
    IF V_RATE > 0 THEN
        V_SECONDS_LEFT := V_UBLK_END / V_RATE;
        DBMS_OUTPUT.PUT_LINE('--------------------------------------------------');
        DBMS_OUTPUT.PUT_LINE('Current Undo Blocks Left : ' || V_UBLK_END);
        DBMS_OUTPUT.PUT_LINE('Rollback Speed           : ' || ROUND(V_RATE, 2) || ' blocks/sec');
        DBMS_OUTPUT.PUT_LINE('Estimated Time Remaining : ' || ROUND(V_SECONDS_LEFT / 60, 2) || ' minutes');
        DBMS_OUTPUT.PUT_LINE('--------------------------------------------------');
    ELSE
        DBMS_OUTPUT.PUT_LINE('Rollback stalled or completed during the wait window.');
    END IF;
EXCEPTION
    WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('Transaction not found. The rollback is fully complete.');
END;
/
=========================================================================================================================================
--Session Waits
...
COL USERNAME FOR A10 HEADING 'USER_ORA'
...
=========================================================================================================================================
--INFO ABOUT LOCKED OBJECTS:
COL SESSION FOR A13
COL "CONNECTION" FOR A15
COL OBJECT FOR A30
COL MACHINE FOR A15
COL COMMAND FOR A15
...
SELECT DISTINCT 
--L.OBJECT_ID, 
 L.SESSION_ID||','||S.SERIAL#||',@'||S.INST_ID AS "SESSION", L.OS_USER_NAME ||'@'|| S.USERNAME AS "CONNECTION", O.OBJECT_NAME AS "OBJECT", S.SQL_ID, MACHINE, C.COMMAND_NAME AS "COMMAND", 
 TO_CHAR(S.LOGON_TIME, 'MMDDYY-HH24:MI:SS') AS "LOGON", TO_CHAR(S.SQL_EXEC_START, 'MMDDYY-HH24:MI:SS') AS "START",
CASE 
    WHEN S.STATUS = 'ACTIVE' THEN 'A'
    WHEN S.STATUS = 'INACTIVE' THEN 'I'
    ELSE 'O'
END AS "STATUS", S.BLOCKING_SESSION AS "BLK_SESS", S.WAIT_CLASS, S.SECONDS_IN_WAIT AS "WAIT"
