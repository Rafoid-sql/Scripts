...
--AND MACHINE NOT IN ('HMRT\SERVER_DELL')
--AND STATUS IN ('ACTIVE','KILLED')
--AND SID IN (4383,2584,2525,4391,2208,2862,2819)
--AND SERIAL# IN (56486)
--AND STATUS IN ('INACTIVE')
...
COL OSUSER FOR A12
COL COMMAND FOR A60
SELECT INST_ID NODE,SID||','||SERIAL# SID_SER,SQL_ID,STATUS,SCHEMANAME||'@'||SERVICE_NAME FROM_WHERE,OSUSER,PROGRAM,NVL((SELECT DISTINCT SQL_TEXT FROM GV$SQL SQL WHERE SQL.SQL_ID = SES.SQL_ID),'NOTHING GOING ON') COMMAND,BLOCKING_SESSION,TO_CHAR(LOGON_TIME) FROM_WHEN
FROM GV$SESSION SES
WHERE TYPE IN ('USER')
...
--AND SCHEMANAME IN ('CERTO2_EXTRACT')
--AND NVL((SELECT DISTINCT SQL_TEXT FROM GV$SQL SQL WHERE SQL.SQL_ID = SES.SQL_ID),'NOTHING GOING ON') NOT LIKE '%NOTHING GOING ON%'
--AND NVL((SELECT DISTINCT SQL_TEXT FROM GV$SQL SQL WHERE SQL.SQL_ID = SES.SQL_ID),'NOTHING GOING ON') like ('%monthly_tap_international%')
ORDER BY STATUS,OSUSER,PROGRAM;
=========================================================================================================================================
-- Check Session Details II
COL FROM_WHERE FOR A35
select distinct MACHINE ,SCHEMANAME||'@'||SERVICE_NAME FROM_WHERE,MACHINE,TERMINAL,PROGRAM from gv$session WHERE TYPE IN ('USER') AND SCHEMANAME IN ('ESP2SCMS','SCMSA_ESP') ORDER BY STATUS,OSUSER,PROGRAM;
=========================================================================================================================================
